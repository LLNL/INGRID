...
def _importBody(gridue_settings, f):
    """
    Imports the body of a gridue file and returns a dictionary with the data.

    Parameters:
    gridue_settings : dict
        Dictionary with header information from the gridue file.

    f : file object
        Opened file object to read the body of the gridue file.

    Returns:
    gridue_settings : dict
        A dictionary containing the gridue body data, with keys corresponding to
        the items in BodyItems and values as numpy arrays.
    """
    next(f)
...
def _importSN(values, f):
    """
    Import a single null file. Only used for Single null divertor topology.
    """
...
def _importDN(values, f):
    """
    Import a double null file. Used for most divertor topologies.
    """
...
def calcMetric(grd: dict, bpsign, verbose=False, ignore_checks=False):
    """
    Calculate metric tensor given BOUT++ cell centers at each grid point.
...
def Convert_grids(gridue_file: str, output_filename: str, plotting: bool = False, verbose: bool = False, ignore_checks: bool = False):
    ...
    Parameters:
    gridue_file : str
        Path to the UEDGE grid file (gridue format).
    output_filename : str
        Path to save the converted BOUT++ grid file.
    plotting : bool, optional
        If True, plot the grid after conversion.
    verbose : bool, optional
        If True, print detailed information about the grid and conversion process.
    ignore_checks : bool, optional
        If True, ignore checks on the Jacobian's relative error.
    Raises:
    -------
    ValueError
        If the magnetic field direction and poloidal magnetic field have opposite signs.
    ValueError
        If the relative error in the Jacobian is too large and ignore_checks is False.
    ValueError
        If the gridue file format is unrecognized.
    ValueError
        If the gridue file does not contain the expected header information.
    ValueError
        If the gridue file does not contain the expected body information.
    ValueError
        If the gridue file does not contain the expected single null or double null topology.
    ValueError
        If the gridue file does not contain the expected number of grid points.
    ValueError
        If the gridue file does not contain the expected number of poloidal or radial points.
    ValueError
        If the gridue file does not contain the expected number of boundary points.
    ValueError
        If the gridue file does not contain the expected number of inner or outer points.
    ValueError
        If the gridue file does not contain the expected number of inner or outer boundary points.
    ValueError
        If the gridue file does not contain the expected number of inner or outer plate points.
    """
