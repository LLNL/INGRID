...
class SNL(TopologyUtils):
    ...
    def AdjustGrid(self) -> None:
        ...
        for patch in self.patches.values():
            ...
            if poloidal_tag == 'C':
                patch.AdjustBorder(
                    'E', [p for p in self.patches.values() if p.get_tag() == 'D' + radial_tag][0])
            ...
            try:
                if patch.TerminatesLoop:
                    ...
                    pname = self.PatchTagMap[self.ConnexionMap.get(patch.get_tag())[
                        'E'][0]]
    ...
    def AdjustPatch(self, patch):
        primary_xpt = Point(
            self.LineTracer.NSEW_lookup['xpt1']['coor']['center'])
    ...
    def construct_patches(self):
        ...
        # Drawing Separatrix
        print("Drawing Separatrix...")
        xptNW_midLine = self.LineTracer.draw_line(xpt['NW'], {
                                                  'line': midline_1}, option='theta', direction='cw', show_plot=visual, text=verbose)
        xptN_psiMinCore = self.LineTracer.draw_line(xpt['N'], {
                                                    'psi': psi_core}, option='rho', direction='cw', show_plot=visual, text=verbose)
        xptNE_midLine = self.LineTracer.draw_line(xpt['NE'], {
                                                  'line': midline_2}, option='theta', direction='ccw', show_plot=visual, text=verbose)
        ...
        if self.settings['grid_settings']['patch_generation']['use_xpt1_W']:
            ...
            d = 'ccw'
            xptW_psiMax = self.LineTracer.draw_line(xpt['W'], {'psi_horizontal': (
                psi_1, tilt)}, option='z_const', direction=d, show_plot=visual, text=verbose)
        else:
            xptW_psiMax = self.LineTracer.draw_line(xpt['W'], {
                                                    'psi': psi_1}, option='rho', direction='ccw', show_plot=visual, text=verbose)

        if self.settings['grid_settings']['patch_generation']['use_xpt1_E']:
            tilt = self.settings['grid_settings']['patch_generation']['xpt1_E_tilt']
            d = 'cw'
            xptE_psiMax = self.LineTracer.draw_line(xpt['E'], {'psi_horizontal': (
                psi_1, tilt)}, option='z_const', direction=d, show_plot=visual, text=verbose)
        else:
            xptE_psiMax = self.LineTracer.draw_line(xpt['E'], {
                                                    'psi': psi_1}, option='rho', direction='ccw', show_plot=visual, text=verbose)

        xpt_WestPlate = self.LineTracer.draw_line(xpt['SW'], {
                                                  'line': WestPlate}, option='theta', direction='ccw', show_plot=visual, text=verbose)
        xptS_psiMinPF = self.LineTracer.draw_line(xpt['S'], {
                                                  'psi': psi_pf_1}, option='rho', direction='cw', show_plot=visual, text=verbose)
        xpt_EastPlate = self.LineTracer.draw_line(xpt['SE'], {
                                                  'line': EastPlate}, option='theta', direction='cw', show_plot=visual, text=verbose)
        iPsiMax_TP = self.LineTracer.draw_line(
            xptW_psiMax.p[-1], {'line': WestPlate}, option='theta', direction='ccw', show_plot=visual, text=verbose)
        psiMinPF_WestPlate = self.LineTracer.draw_line(
            xptS_psiMinPF.p[-1], {'line': WestPlate}, option='theta', direction='ccw', show_plot=visual, text=verbose)
        oPsiMax_TP = self.LineTracer.draw_line(
            xptE_psiMax.p[-1], {'line': EastPlate}, option='theta', direction='cw', show_plot=visual, text=verbose)
        psiMinPF_EastPlate = self.LineTracer.draw_line(
            xptS_psiMinPF.p[-1], {'line': EastPlate}, option='theta', direction='cw', show_plot=visual, text=verbose)
        ...
        # =====================================================================================
        A2_W = (WestPlate.split(A2_S.p[-1])[1]
                ).split(A2_N.p[0], add_split_point=True)[0]
        A2 = Patch([A2_N, A2_E, A2_S, A2_W], patch_name='A2',
        ...
        A1_E = xptS_psiMinPF
        A1_W = (WestPlate.split(A1_S.p[-1])[1]
                ).split(A1_N.p[0], add_split_point=True)[0]
        A1 = Patch([A1_N, A1_E, A1_S, A1_W], patch_name='A1',
                   plate_patch=True, plate_location=location)

        # B2 Patch

        B2_N = self.LineTracer.draw_line(
            A2_N.p[-1], {'line': midline_1}, option='theta', direction='cw', show_plot=visual, text=verbose)
        ...
        B1_S = self.LineTracer.draw_line(
            xptN_psiMinCore.p[-1], {'line': midline_1}, option='theta', direction='cw', show_plot=visual, text=verbose).reverse_copy()
        B1_E = Line([B1_N.p[-1], B1_S.p[0]])
        B1_W = xptN_psiMinCore.reverse_copy()
        B1 = Patch([B1_N, B1_E, B1_S, B1_W], patch_name='B1')

        # C2 Patch
        C2_N = self.LineTracer.draw_line(
            B2_N.p[-1], {'line': topLine}, option='theta', direction='cw', show_plot=visual, text=verbose)
        ...
        C1_S = self.LineTracer.draw_line(B1_S.p[0], {
                                         'line': topLine}, option='theta', direction='cw', show_plot=visual, text=verbose).reverse_copy()
        ...
        F2_E = (EastPlate.split(F2_N.p[-1])[1]
                ).split(F2_S.p[0], add_split_point=True)[0]
        F2_W = xptE_psiMax
        ...
        F1_E = (EastPlate.split(F1_N.p[-1])[1]
                ).split(F1_S.p[0], add_split_point=True)[0]
        F1_W = xptS_psiMinPF.reverse_copy()
        F1 = Patch([F1_N, F1_E, F1_S, F1_W], patch_name='F1',
                   plate_patch=True, plate_location=location)

        # E2 Patch
        E2_N = self.LineTracer.draw_line(F2_N.p[0], {
                                         'line': midline_2}, option='theta', direction='ccw', show_plot=visual, text=verbose).reverse_copy()
        E2_S = xptNE_midLine
        ...
        E1_S = self.LineTracer.draw_line(
            xptN_psiMinCore.p[-1], {'line': midline_2}, option='theta', direction='ccw', show_plot=visual, text=verbose)
        E1_E = xptN_psiMinCore
        E1_W = Line([E1_S.p[-1], E1_N.p[0]])
        E1 = Patch([E1_N, E1_E, E1_S, E1_W], patch_name='E1')

        # D2 Patch
        D2_N = self.LineTracer.draw_line(E2_N.p[0], {
                                         'line': topLine}, option='theta', direction='ccw', show_plot=visual, text=verbose).reverse_copy()
        ...
        D1_S = self.LineTracer.draw_line(
            E1_S.p[-1], {'line': topLine}, option='theta', direction='ccw', show_plot=visual, text=verbose)
    ...
    def OrderPatches(self):
        patches = ['A2', 'B2', 'C2', 'D2', 'E2',
                   'F2', 'A1', 'F1', 'B1', 'C1', 'D1', 'E1']
        self.patches = OrderedDict(
            [(pname, self.patches[pname]) for pname in patches])
