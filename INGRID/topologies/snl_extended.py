"""
The ``snl_extended`` module contains :class:`SNL_extended` for representing a single-null
...
class SNL_extended(TopologyUtils):
    """
    The SNL_extended class for handling `Lower Single Null` (LSN) and
    ...
    def construct_patches(self):
        ...
        #
        # Generate the remaining patches which extend to the vessel wall
        #
        # Drawing Lower-SNL region
        if self.settings['grid_settings']['patch_generation']['use_xpt1_W']:
            pass
            #
            # FIXME: We need to allow for terminating via line intersection but when tracing a straight line
            #
            tilt = self.settings['grid_settings']['patch_generation']['xpt1_W_tilt']
            d = 'ccw'
            A3_E = self.LineTracer.draw_line(xptW_psiMax.p[-1], {'psi_horizontal': (psi_1, tilt)}, option='z_const', direction=d, show_plot=visual, text=verbose)

        A3_E = self.LineTracer.draw_line(xptW_psiMax.p[-1], {'line': WestPlate}, option='rho', direction='ccw', show_plot=visual, text=verbose)
