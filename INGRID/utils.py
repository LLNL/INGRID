    ...
    **kwargs :
    ...
    def SetDefaultSettings(self) -> None:
        ...
        self.default_values_lookup = {
            ...
            'virtual': self.default_virtual_bounds,
    ...
    def ProcessPaths(self) -> None:
        ...
        for k in self.default_dir_settings.keys():
            ...
            if k == 'limiter':
                ...
                continue
            if k == 'virtual':
                self.settings['virtual']['file'] = str((path_obj / self.settings['virtual']['file']).absolute())
                continue
            if k == 'target_plates':
    ...
    def SetVirtualBoundary(self, fpath: str = '', coordinates: list = [], rshift: float = 0.0, zshift: float = 0.0) -> None:
        """
        Instantiate the class Line object that represents the tokamak limiter.
        ...
        rshift = self.settings['virtual'].get('rshift', rshift)
        zshift = self.settings['virtual'].get('zshift', zshift)

        if fpath not in ['', '.']:
            print('# Processing file for virtual boundary data : {}'.format(fpath))
            rpts, zpts = self.ParseTxtCoordinates(fpath)

        self.VirtualBoundary = Line([Point(p) for p in zip(np.array(rpts) + rshift, np.array(zpts) + zshift)])
    ...
    def ClassifyTopology(self, visual=False) -> None:
        ...
        if self.config == 'UDN' and not self.settings.get('virtual', {}).get('file', '') in ['', '.']:
            self.config = 'UDN_extended'
...
class TopologyUtils():
    ...
    def patch_diagram(self, fig: object = None, ax: object = None) -> None:
        ...
        alpha = {'3': 1.0, '2': 0.75, '1': 0.5, '4': 0.25}
    ...
    def grid_diagram(self, fig: object = None, ax: object = None) -> None:
        ...
        self.parent.LimiterData.plot(ax=ax, color='r', linewidth=2.0)
